...
import com.tiremgmt.vehiclepartsordersvc.service.MemberServiceProxy;
...
public class OrderServiceImpl implements OrderService {
	...
	@Override
	public String saveOrder(OrdersDTO ordersDTO,String authorization) {
		if (ordersDTO == null)
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, Constants.INVALID_ORDER_DETAILS);
		MemberDTO memberDTO = getMemberByUsername(ordersDTO.getUsername(),authorization);
	...
	private MemberDTO getMemberByUsername(String username,String authorization) {
		String memberByUsernameUrl = userByNameMemberUrl + username;//Constants.MEMBER_SERVICE_GET_MEMBER_BY_USERNAME_URL + username;
		log.info("memberByUsernameUrl : {}", memberByUsernameUrl);
		MemberDTO memberDTO = null;
		try {
			ResponseEntity<MemberDTO> responseEntity = memberServiceProxy.getMemberDetailsByUsername(authorization,username);
//			ResponseEntity<MemberDTO> responseEntity = restTemplate.getForEntity(memberByUsernameUrl, MemberDTO.class);
//			ResponseEntity<MemberDTO> responseEntity = restTemplate.exchange(memberByUsernameUrl, HttpMethod.GET, setHttpHeaders(authorization), MemberDTO.class);
	...
	@Override
	public MemberDTO getAllOrdersByUsername(String username,String authorization) {
		MemberDTO memberDTO = getMemberByUsername(username,authorization);
	...
	public MemberDTO getOrderByNumber(String orderNumber,String authorization) {
		...
		Member m = orders.get().getMember();
		MemberDTO memberDTO = getMemberByUsername(orders.get().getMember().getUsername(),authorization);
